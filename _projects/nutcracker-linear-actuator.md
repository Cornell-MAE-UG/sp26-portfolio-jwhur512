---
layout: project
title: Nutcracker Design
description: Design a nutcracker using a linear actuator and beam bending analysis
technologies: Mechanism, Machine
image: /assets/images/nutcracker-linak.png
---

As part of our class problem set/design problem, we were assigned to design a nutcracker under a specific restriction. This is how I approached the design problem.

Find:
- The necessary dimensions of the nutcraker that abides the specific input parameters-including a diagram.

Given:
- The researched typical values:
    - Diameter of macadamia nut: 25 mm
    - F(crack) = 2500 N
    - F(grip) = 200 N

Solution:
![A written solution with a drawn diagram of the nutcracker design problem]({{ '/assets/images/nutcracker-beam.jpg' | relative_url }}){: .inline-image-r
style="width: 800px"}

